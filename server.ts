...
// 1d. Universal AI-Adaptive Business Catalog Schema
// Replaces hardcoded per-industry product assumptions with a business-specific
// schema generated by the SAME unified engine (DeepSeek V4 Pro -> Gemini).
...
app.post('/api/ai/catalog-schema', async (req, res) => {
  ...
  const catalogReq: CatalogRequest = {
    businessRequirements: String(
      body.businessRequirements || body.requirements || body.description || body.industry || ''
    ).slice(0, 4000),
    businessName: typeof body.businessName === 'string' ? body.businessName.slice(0, 120) : undefined,
    industry: typeof body.industry === 'string' ? body.industry.slice(0, 120) : undefined,
    businessModel: typeof body.businessModel === 'string' ? body.businessModel.slice(0, 60) : undefined,
    currency: typeof body.currency === 'string' ? body.currency.slice(0, 10) : undefined,
    country: typeof body.country === 'string' ? body.country.slice(0, 80) : undefined,
    existingNotes: typeof body.existingNotes === 'string' ? body.existingNotes.slice(0, 800) : undefined,
  };

  if (!catalogReq.businessRequirements.trim()) {
    return res.json({
      success: false,
  ...
  try {
    ...
    return res.json({
      success: true,
      schema,
      provider: schema.provider || 'deepseek',
      modelUsed: schema.model || engine.id,
    ...
    return res.json({
      success: true,
      schema: neutralFallbackSchema(catalogReq),
      provider: 'fallback',
      modelUsed: 'neutral-fallback',
      creditsUsed: 0,
      degraded: true,
      message: error?.message || 'AI catalog generation failed; a neutral schema was provided.',
    });
  }
});
